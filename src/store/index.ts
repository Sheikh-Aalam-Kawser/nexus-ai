...
interface AppState {
  ...
  draftGoogleDoc: (title: string, content: string) => Promise<void>;
  ingestCalendarEvents: () => Promise<{imported: number; emergencies: number}>;
}
...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      generateProposedPlan: async () => {
        ...
        try {
          ...
          for (const t of sortedPending) {
            ...
            if (durationMinutes <= 0) durationMinutes = 15; // fallback
      ...
      draftGoogleDoc: async (title: string, content: string) => {
        const token = await getAccessToken();
        if (!token) {
          console.error("No access token for Google Docs");
          get().addPlanExecutionLog("Failed to draft Google Doc: No access token available.");
          return;
        }

        get().addPlanExecutionLog(`Drafting document: "${title}"...`);
        ...
        try {
          ...
          get().addPlanExecutionLog(`Successfully created Google Doc: "${title}".`);
        } catch (error) {
          console.error("Google Docs error", error);
          get().addPlanExecutionLog("Failed to draft Google Doc.");
        }
      },
      ingestCalendarEvents: async () => {
        let imported = 0;
        let emergencies = 0;
        
        const token = await getAccessToken();
        if (!token) {
          console.error("No access token for Calendar ingest");
          return { imported, emergencies };
        }

        try {
          const timeMin = new Date().toISOString();
          const response = await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMin=${encodeURIComponent(timeMin)}&maxResults=50&singleEvents=true&orderBy=startTime`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
          ...
          events.forEach((event: any) => {
            ...
            // Avoid duplicates by simple summary match
            if (currentTasks.some(t => t.title === event.summary)) return;
            if (newTasks.some(t => t.title === event.summary)) return;
            // Ignore events that are full day (without time) or generated by NEXUS
            if (event.summary.startsWith('[NEXUS]')) return;
            
            const startStr = event.start?.dateTime || event.start?.date;
            if (!startStr) return;
            
            const deadlineDate = new Date(startStr);
            const timeDiff = deadlineDate.getTime() - Date.now();
            const hoursDiff = timeDiff / (1000 * 3600);
            
            // Only care about events that could be tasks (e.g. within next 30 days)
            if (hoursDiff > 30 * 24) return;
            
            let priority = 50;
            if (hoursDiff < 48) priority = 80;
            if (hoursDiff < 24) {
              priority = 95;
              emergencies++;
            }
            ...
            // Add as task
            const userId = get().user?.uid || "unknown-user";
            newTasks.push({
          ...
          if (newTasks.length > 0) {
            set((state) => ({ tasks: [...state.tasks, ...newTasks], isFocusPlanPopupVisible: true }));
            get().autoEvaluatePrioritiesAndFocus();
            get().triggerAutoAIPlan().catch(err => console.warn("Auto AI Prioritize failed:", err)).finally(() => {
              get().generateProposedPlan().then(() => {
                const state = get();
                if (state.proposedPlan) {
                  state.approvePlan();
